...
import { DEFAULT_SETTINGS, SettingsSchema, type AuthorEntry, type Settings, type UiStats } from "./types";
...
export const startXide = async () => {
  ...
  const handleShimEntries = async (entries: ShimHit[]) => {
    ...
    for (const entry of entries) {
      ...
      const needsApi = existing === null || (existing.country === null && cache.isStale(existing));
