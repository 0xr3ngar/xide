...
const createChip = (label: string, count: number | null, onRemove: () => void, removeLabel: string) => {
...
const renderCountryChips = (container: HTMLElement, ctx: UiContext) => {
  ...
  for (const code of blocked) {
    ...
    const chip = createChip(name, count, () => {
      const next = { ...ctx.settings(), blocked: ctx.settings().blocked.filter((c) => c !== code) };
...
const renderAllowChips = (container: HTMLElement, ctx: UiContext) => {
  ...
  for (const handle of ctx.settings().allowlist) {
    const chip = createChip(`@${handle}`, null, () => {
      const next = { ...ctx.settings(), allowlist: ctx.settings().allowlist.filter((h) => h !== handle) };
      ctx.saveSettings(next);
...
const searchMatches = (query: string, ctx: UiContext) => {
  ...
  return COUNTRIES.filter((c) => !blocked.has(c.code) && c.name.toLowerCase().includes(q)).slice(0, 6);
}
...
const createPanel = (ctx: UiContext) => {
  ...
  if (stats === null || search === null || drop === null || chips === null || allowInput === null || allowChips === null || closeBtn === null) return null;
  ...
  search.addEventListener("input", () => {
    ...
    renderDropdown(drop, matches, pick, { top: rect.bottom, left: rect.left, width: rect.width });
  });
